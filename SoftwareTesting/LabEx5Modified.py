...
# --------------------------------------------
# CASE Tool Simulation: Persistent Defect Tracking System
# --------------------------------------------

DATA_FILE = "defects.json"

# Step 1: Load or initialize defect database
def load_defects():
...
def save_defects(defect_database):
    with open(DATA_FILE, "w") as file:
        json.dump(defect_database, file, indent=4)

# Initialize defect database
defect_database = load_defects()
defect_id_counter = max(map(int, defect_database.keys()), default=0) + 1  # Auto-incrementing defect ID

# Step 2: Function to add a new defect
def add_defect(title, description, severity):
    global defect_id_counter
    if severity not in ["Low", "Medium", "High"]:
        print("Error: Severity must be Low, Medium, or High.")
        return

    defect_database[str(defect_id_counter)] = {
        "Title": title,
        "Description": description,
        "Severity": severity,
        "Status": "Open"
    }
    save_defects(defect_database)
